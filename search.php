...
if (!isset($_GET['town_id'])) {
    die("Please select a valid town to be able to search.");
}
$townId = $_GET['town_id'];

$searchTown = mysqli_query($connection,"SELECT * FROM locations WHERE id='$townId'") or die("Failed to get the town details.".mysqli_error($connection));

if (mysqli_num_rows($searchTown) < 1) {
 die("Please select a valid town to be able to search.");
}

$searchTown = mysqli_fetch_assoc($searchTown);


$googleApiUrl = "https://api.openweathermap.org/data/3.0/onecall?lat=".$searchTown['lat']."&lon=".$searchTown['lng']."&units=metric&country=KE&exclude=&appid=".$apiKey;
...
<section class="section section__contact pt-4">
    <div class="container">
     ...
     <h4 class="pt-3">Daily weather forecast for <strong><?php echo $searchTown['town'];?></strong></h4>
          ...
          <div class="form-group col-md-8">
            ...
            <select name="town_id" class="form-control" style="padding: 5px !important">
                <?php  foreach($towns as $town) { ?>
                    <option <?php if (isset($_GET['town_id'])) {
                     if ($_GET['town_id'] == $town['id']) {
                         echo "selected";
                     }
                 } ?> value="<?php echo $town['id'] ?>"><?php echo $town['town']; ?></option>
             <?php } ?>
         </select>
     </div>
   ...
   <div class="col-lg-6">
    ...
    <!--                    <div class="row ">-->
        <div class="report-container">
            <h2><?php echo $searchTown['town']; ?> Weather Status</h2>
            <div class="time">
...
<div class="row">
    <div class="col-lg-12">
        <h2><?php echo $searchTown['town']; ?> Hourly weather forecast Status</h2>
        <table class="table">
